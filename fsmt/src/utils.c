...
int fsmt_circle_ray_intersection(const fsmt_cartesian_point_t *po, float angle, float r,
    ...
    float ux = cosf(angle);
    float uy = sinf(angle);
    // Precompute dot product and squared norms
    float dot = po->x * ux + po->y * uy;
    float norm_sq = po->x * po->x + po->y * po->y;

    // Compute discriminant
    float discriminant = dot * dot - (norm_sq - r * r);
    ...
    // Take the positive root (moving forward along direction)
    float t = -dot + sqrt(discriminant);

    // Compute intersection point
    p_circle->x = po->x + t * ux;
    p_circle->y = po->y + t * uy;
